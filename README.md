...
Since 0.1.0.9003, the trusted client creates a private temporary directory and sends base64 JSON to `dataraft.ide::ide_request(encoded, context = dataraft.ide::ide_context(response_root = private))`. The trusted context sets the response root, defaulting to the R session temporary directory. Requests cannot override it. Canonical path containment rejects writes outside that root, including sibling-prefix and symlink escapes, before dispatch and again before writing. Requests specify numeric `version: 1`, `operation`, `request_id` and an unused absolute `response_path`. The response is a complete envelope with numeric `contract: 1`, timestamp, kind, matching request ID, data and error. The writer atomically renames a temporary sibling file. Existing files and links are rejected. Clients must bound file reads, validate the bundled JSON Schema and match request IDs; console output is not a transport.
...
The optional adapters package supplies safe ODCS 3.2 contract import. `profile` returns only an in-memory table's column names and types. `validate_contract` checks a saved YAML file of at most 1 MiB. Since 0.1.0.9004, both it and `sample_quality` require canonical containment inside `ide_context(read_roots = ...)`. The default captures the current working directory, and `character()` disables contract reads. Configure additional roots in trusted R code, never in encoded input. Existing roots are rechecked to reject symlink replacement; sibling-prefix and symlink escapes are rejected. This is a local request boundary, not an OS sandbox against code running as the same user. `sample_quality` evaluates that contract against the first bounded rows of an explicit in-memory table and returns aggregate evidence. The bridge never guesses editor source positions.
